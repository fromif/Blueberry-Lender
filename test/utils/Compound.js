...
async function makeBToken(opts = {}) {
  ...
  switch (kind) {
    case 'cether':
      bToken = await deploy('CEtherHarness',
        [
    ...
    case 'ccapable':
      ...
      bToken = await saddle.getContractAt('CCapableErc20Delegate', cDelegator._address);
      break;
    ...
    case 'ccollateralcap':
      ...
      bToken = await saddle.getContractAt('CCollateralCapErc20DelegateHarness', cDelegator._address);
    ...
    case 'ccollateralcapnointerest':
      ...
      bToken = await saddle.getContractAt('CCollateralCapErc20NoInterestDelegateHarness', cDelegator._address);
      version = 1; // ccollateralcap's version is 1
      break;
    ...
    case 'cwrapped':
      ...
      bToken = await saddle.getContractAt('CWrappedNativeDelegateHarness', cDelegator._address); // XXXS at
    ...
    case 'cevil':
      ...
      bToken = await saddle.getContractAt('CCollaterlaCapErc20CheckRepayDelegateHarness', cDelegator._address); // XXXS at
      version = 1; // ccollateralcap's version is 1
      break;
    ...
    default:
      ...
      bToken = await saddle.getContractAt('CErc20DelegateHarness', cDelegator._address); // XXXS at
      break;
  }
...
async function makeEvilAccount(opts = {}) {
  const crEth = opts.crEth || await makeBToken({kind: 'cether'});
  const crEvil = opts.crEvil || await makeBToken({kind: 'cevil'});
...
async function makeEvilAccount2(opts = {}) {
  const crWeth = opts.crWeth || await makeBToken({kind: 'cerc20'});
  const crEvil = opts.crEvil || await makeBToken({kind: 'cevil'});
