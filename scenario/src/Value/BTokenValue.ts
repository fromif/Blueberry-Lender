...
export async function getBErc20DelegatorV(
  world: World,
  event: Event
): Promise<BErc20Delegator> {
  const address = await mapValue<AddressV>(
    world,
    event,
    (str) => new AddressV(getBTokenAddress(world, str)),
  ...
  return getWorldContractByAddress<BErc20Delegator>(world, address.val);
}
...
async function getBTokenPendingAdmin(
  world: World,
  bToken: BToken
): Promise<AddressV> {
...
async function getReserveFactor(
  ...
  return new NumberV(
    await bToken.methods.reserveFactorMantissa().call(),
    1.0e18
  );
}
...
async function getInterestRate(world: World, bToken: BToken): Promise<NumberV> {
  return new NumberV(
    await bToken.methods.borrowRatePerBlock().call(),
    1.0e18 / 2102400
  );
}
...
async function getImplementation(
  ...
  return new AddressV(
    await (bToken as BErc20Delegator).methods.implementation().call()
  );
}
...
async function getAccountCollateralToken(
  ...
  return new NumberV(await bToken.methods.accountCollateralTokens(user).call());
}
...
async function getTotalCollateralTokens(
  ...
  return new NumberV(await bToken.methods.totalCollateralTokens().call());
}
...
export function bTokenFetchers() {
  return [
    ...
    new Fetcher<{ bToken: BToken }, AddressV>(
      ...
      [new Arg("bToken", getBTokenV)],
      async (world, { bToken }) =>
        new AddressV(await bToken.methods.underlying().call()),
      { namePos: 1 }
    ),
    ...
    new Fetcher<{ bToken: BToken; address: AddressV }, NumberV>(
      ...
      (world, { bToken, address }) =>
        balanceOfUnderlying(world, bToken, address.val),
      { namePos: 1 }
    ),
    ...
    new Fetcher<{ bToken: BToken; address: AddressV }, NumberV>(
      ...
      (world, { bToken, address }) =>
        getBorrowBalance(world, bToken, address.val),
      { namePos: 1 }
    ),
    ...
    new Fetcher<{ bToken: BToken; address: AddressV }, NumberV>(
      ...
      [new Arg("bToken", getBTokenV), new Arg("address", getAddressV)],
      (world, { bToken, address }) =>
        getBorrowBalanceStored(world, bToken, address.val),
      { namePos: 1 }
    ),
    ...
    new Fetcher<{ bToken: BToken; address: AddressV }, NumberV>(
      ...
      [new Arg("bToken", getBTokenV), new Arg("address", getAddressV)],
      (world, { bToken, address }) =>
        getAccountCollateralToken(world, bToken, address.val),
      { namePos: 1 }
    ),
    new Fetcher<{ bToken: BToken }, NumberV>(
      `
      ...
      "TotalCollateralTokens",
      [new Arg("bToken", getBTokenV)],
      (world, { bToken }) => getTotalCollateralTokens(world, bToken),
      { namePos: 1 }
    ),
  ];
}

export async function getBTokenValue(
  world: World,
  event: Event
): Promise<Value> {
