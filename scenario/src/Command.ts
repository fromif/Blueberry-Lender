        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (BToken Eth Address) (... deploy bToken)`
              // could be used to deploy a bToken if it doesn't exist, but
              // since there is no BToken, that check would raise (when we'd
