...
describe('BToken', function () {
  ...
  describe('_setComptroller', () => {
    it("should fail if called by non-admin", async () => {
      expect(
        await send(bToken, '_setComptroller', [newComptroller._address], { from: accounts[0] })
    ...
    it("reverts if passed a contract that doesn't implement isComptroller", async () => {
      await expect(send(bToken, '_setComptroller', [bToken.underlying._address])).rejects.toRevert("revert");
      expect(await call(bToken, 'comptroller')).toEqual(oldComptroller._address);
    });
    ...
    it("reverts if passed a contract that implements isComptroller as false", async () => {
      ...
      await expect(send(bToken, '_setComptroller', [badComptroller._address])).rejects.toRevert("revert not comptroller");
      expect(await call(bToken, 'comptroller')).toEqual(oldComptroller._address);
    });

    it("updates comptroller and emits log on success", async () => {
      const result = await send(bToken, '_setComptroller', [newComptroller._address]);
