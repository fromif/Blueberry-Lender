...
import "./BToken.sol";
import "./BTokenCheckRepay.sol";
import "./ERC3156FlashLenderInterface.sol";
import "./ERC3156FlashBorrowerInterface.sol";

/**
 * @title Blueberry's BCollateralCapErc20CheckRepay Contract
 * @notice BTokens which wrap an EIP-20 underlying with collateral cap
 * @author Blueberry
 */
contract BCollateralCapErc20CheckRepay is BTokenCheckRepay, BCollateralCapErc20Interface {
    /**
    ...
    function initializeAccountCollateralTokens(address account) internal {
        ...
        if (!isCollateralTokenInit[account]) {
            if (ComptrollerInterfaceExtension(address(comptroller)).checkMembership(account, BToken(address(this)))) {
    ...
    function mintFresh(
         ...
         */
        if (ComptrollerInterfaceExtension(address(comptroller)).checkMembership(minter, BToken(address(this)))) {
